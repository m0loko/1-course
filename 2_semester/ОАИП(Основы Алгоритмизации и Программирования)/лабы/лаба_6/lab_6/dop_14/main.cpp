...
void insert(list*& p, int value) {
	...
	{
		cout << "Операция доавления не выполнена\n";
	}
}
bool isEmpty(list* p) {
	return p == NULL;
...
void toFile(list* p) {
	...
	frm.close();
	cout << "Список записан в list.txt\n";
}
...
void menu(void)
{
	...
	cout << " 7 - Найти сумму отрицательных элементов, кратных 5, или выдать сообщение, что таких элементов нет" << endl;
	cout << " 8 - Выход" << endl;
}
void myVar(list* p) {
	int sum = 0;
	...
	else
	{
		...
		{
			cout << "Отрицательных элементов, кратных 5 нет" << endl;
		}
	}
}
int main() {
